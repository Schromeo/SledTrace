...
- first intended production PyPI release for ordinary `pip install sledtrace`
...
Production publication is complete only after the immutable `v0.7.0` tag runs through the protected `pypi` environment and a clean install from production PyPI succeeds. The release workflow and final validation record are added to the GitHub Release after that gate passes.
